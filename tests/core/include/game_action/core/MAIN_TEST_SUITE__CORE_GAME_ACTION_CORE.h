#include "test_suite_game_action_core_game_action__bad_request.h"
#include "tcp/MAIN_TEST_SUITE__CORE_GAME_ACTION_CORE_TCP.h"
#include "entity/MAIN_TEST_SUITE__CORE_GAME_ACTION_CORE_ENTITY.h"
#include "global_space/MAIN_TEST_SUITE__CORE_GAME_ACTION_CORE_GLOBAL_SPACE.h"
#include "hitbox/MAIN_TEST_SUITE__CORE_GAME_ACTION_CORE_HITBOX.h"

#include <test_util.h>

DECLARE_SUITE(CORE_GAME_ACTION_CORE);

