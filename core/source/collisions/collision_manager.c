...
static inline
Collision_Node_Record *get_p_collision_node_record_by__index_in__collision_node(
    ...
    if (index_of__record
        ...
        debug_error("get_p_collision_node_record_by__index_in__collision_node, index out of bounds: %d/%d",
                index_of__record,
                ENTITY_MAXIMUM_QUANTITY_OF__COLLIDABLE);
        return 0;
    }
    ...
    return &p_collision_node
        ->collision_node_records[index_of__record];
}
...
void initialize_collision_manager__collision_node(
    ...
    for (uint32_t i=0;
            ...
            i++) {
        Collision_Node_Record *p_collision_node_record =
            get_p_collision_node_record_by__index_in__collision_node(
                    collision_node, i);
        p_collision_node_record->p_hitbox = 0;
    }
}
...
void check_collision_node_for__collisions(
        ...
        f_Collision_Handler f_collision_handler) {
    for (uint32_t i=0;
            i<ENTITY_MAXIMUM_QUANTITY_OF__COLLIDABLE;
            i++) {
        Collision_Node_Record *p_collision_node_record =
        ...
        if (!p_collision_node_record->p_hitbox) {
            return;
        }

        if (p_collision_node_record
                ->s_data
        ...
        if (direction_of_collision != DIRECTION__NONE) {
            f_collision_handler(
                    ...
                    p_source,
                    p_hitbox,
                    &p_collision_node_record->s_data,
                    p_collision_node_record->p_hitbox,
...
void remove_record_from__collision_node(
        Collision_Manager__Collision_Node *p_collision_node,
        Hitbox_AABB *p_hitbox) {

    if (!p_collision_node
            ->collision_node_records[0].p_hitbox) {
...
empty collision node.");
    ...
    Collision_Node_Record *p_removed_record = 0;
    Collision_Node_Record *p_last_record = 0;
    for (uint32_t i=0;
            i<ENTITY_MAXIMUM_QUANTITY_OF__COLLIDABLE;
            i++) {
        Collision_Node_Record *p_collision_node_record =
            get_p_collision_node_record_by__index_in__collision_node(
                    p_collision_node, i);
        if (p_collision_node_record->p_hitbox == p_hitbox) {
            p_removed_record =
                p_collision_node_record;
            break;
        }
...
collision node.");
    ...
    if (p_removed_record == p_last_record) {
        p_removed_record->p_hitbox = 0;
        return;
    }
    ...
    *p_removed_record =
        *p_last_record;
    p_last_record->p_hitbox = 0;
}

Serialized_Field *get_s_record_from__collision_manager_with__3i32F4(
        Collision_Manager *p_collision_manager,
    ...
    for (uint32_t i=0;
        ...
        if (is_hitbox_containing__our_position)
            return &p_collision_node_record->s_data;
    }
    return 0;
}

void foreach_collision_record_within__hitbox(
        Collision_Manager *p_collision_manager,
        Hitbox_AABB *p_hitbox,
        f_Foreach_Serialized_Field f_callback,
        Game *p_game,
        void *p_data) {
    Chunk_Vector__3i32 chunk_vector__3i32 =
        vector_3i32F4_to__chunk_vector_3i32(
...
void unload_all_entities_from__collision_node(
    ...
    for (uint32_t i=0;
            ...
            i++) {
        collision_node
            ->collision_node_records[i]
            .p_hitbox = 0;
    }
}

void add_record_to__collision_node(
        Collision_Manager__Collision_Node *collision_node,
        Serialization_Header *p_object,
        Hitbox_AABB *p_hitbox) {
...
bool add_record_to__collision_manager(
        Collision_Manager *collision_manager,
        Serialization_Header *p_object,
        Hitbox_AABB *p_hitbox) {
    Chunk_Vector__3i32 chunk_vector__3i32 =
...
void remove_record_from__collision_manager__at(
        Collision_Manager *collision_manager,
        Hitbox_AABB *p_hitbox,
        Chunk_Vector__3i32 chunk_vector__3i32) {
...
out of bounds: (%d, %d)", old_x__chunk, old_y__chunk);
    ...
    remove_record_from__collision_node(
            collision_node,
            p_hitbox);
}
