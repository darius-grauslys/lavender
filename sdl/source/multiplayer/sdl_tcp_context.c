...
PLATFORM_TCP_Socket *get_p_PLATFORM_tcp_socket_by__index_from__context(
    ...
    if (index_of__PLATFORM_tcp_socket >
        ...
        debug_error("SDL::get_p_PLATFORM_tcp_socket_by__index_from__context, index out of bounds, %d/%d", index_of__PLATFORM_tcp_socket, MAX_QUANTITY_OF__TCP_SOCKETS);
        return 0;
    }
#endif
...
void SDL_initialize_PLATFORM_tcp_context(
        PLATFORM_TCP_Context *p_PLATFORM_tcp_context) {
...
PLATFORM_TCP_Socket *SDL_allocate_PLATFORM_TCP_Socket_from__context(
    ...
    for (Index__u32 index_of__PLATFORM_tcp_socket = 0;
        ...
        if (SDL_is_PLATFORM_tcp_socket__in_use(
    ...
    if (!p_PLATFORM_tcp_socket__available) {
        debug_error("SDL_allocate_PLATFORM_TCP_Socket_from__context, too many active connections.");
        return 0;
    }
...
PLATFORM_TCP_Socket *PLATFORM_tcp_connect(
    ...
    PLATFORM_TCP_Socket *p_PLATFORM_tcp_socket =
        SDL_allocate_PLATFORM_TCP_Socket_from__context(
                p_PLATFORM_tcp_context);

    if (!p_PLATFORM_tcp_socket) {
        debug_error("SDL::PLATFORM_tcp_connect, failed to get socket.");
        return 0;
    }
...
PLATFORM_TCP_Socket *PLATFORM_tcp_server(
    ...
    if (!p_PLATFORM_tcp_socket) {
        debug_error("SDL::PLATFORM_tcp_server, failed to get socket.");
        return 0;
    }
    ...
    if (listen(p_PLATFORM_tcp_socket->socket_handle,
        ...
        debug_error("SDL::PLATFORM_tcp_server, failed to listen.");
        return 0;
    }
...
PLATFORM_TCP_Socket *PLATFORM_tcp_poll_accept(
    ...
    if (socket_handle__new_connection == -1) {
        if (errno != EWOULDBLOCK && errno != EAGAIN) {
            debug_error("SDL::PLATFORM_tcp_poll_accept, error.");
            return 0;
        }
        return 0;
    }
